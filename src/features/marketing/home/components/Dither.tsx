...
float fbm(vec2 p) {
  float value = 0.0;
  float amp = 1.0;
  float freq = 3.0; // Fixed local base frequency variable reference
  for (int i = 0; i < OCTAVES; i++) {
    value += amp * abs(cnoise(p));
    p *= freq;
    amp *= 0.5; // Fixed local base amplitude factor decay
  }
  return value;
}

float pattern(vec2 p, float timeVal, float speedVal) {
  vec2 p2 = p - timeVal * speedVal;
  return fbm(p + fbm(p2)); 
}
...
void main() {
  ...
  float f = pattern(uv, time, waveSpeed);
